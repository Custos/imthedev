...
class CoreFacade:
    ...
    def _subscribe_to_core_events(self) -> None:
        ...
        # Project events
        self.event_bus.subscribe(EventTypes.PROJECT_CREATED, self._handle_project_created)
        self.event_bus.subscribe(EventTypes.PROJECT_SELECTED, self._handle_project_selected)
        self.event_bus.subscribe(EventTypes.PROJECT_DELETED, self._handle_project_deleted)
        
        # Command events
        self.event_bus.subscribe(EventTypes.COMMAND_PROPOSED, self._handle_command_proposed)
        self.event_bus.subscribe(EventTypes.COMMAND_APPROVED, self._handle_command_approved)
        self.event_bus.subscribe(EventTypes.COMMAND_REJECTED, self._handle_command_rejected)
        self.event_bus.subscribe(EventTypes.COMMAND_EXECUTING, self._handle_command_executing)
        self.event_bus.subscribe(EventTypes.COMMAND_COMPLETED, self._handle_command_completed)
        self.event_bus.subscribe(EventTypes.COMMAND_FAILED, self._handle_command_failed)
        
        # State events
        self.event_bus.subscribe(EventTypes.STATE_AUTOPILOT_ENABLED, self._handle_autopilot_enabled)
        self.event_bus.subscribe(EventTypes.STATE_AUTOPILOT_DISABLED, self._handle_autopilot_disabled)
        self.event_bus.subscribe(EventTypes.STATE_MODEL_CHANGED, self._handle_model_changed)
    ...
    async def create_project(self, name: str, path: str, settings: Optional[ProjectSettings] = None) -> Project:
    ...
    async def select_project(self, project_id: UUID) -> None:
        ...
        context = await self.context_service.load_context(project_id)
        # Update state
    ...
    async def get_pending_commands(self) -> List[Command]:
        ...
        """
        return await self.command_engine.get_pending_commands()
    ...
    async def initialize(self) -> None:
        ...
        if state.current_project_id:
            try:
                context = await self.context_service.load_context(state.current_project_id)
            except Exception:
