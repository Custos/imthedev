...
class StateManagerImpl(StateManager):
    ...
    def __init__(
        self, event_bus: EventBus, state_file_path: Path | None = None
    ) -> None:
    ...
    async def update_state(self, updates: dict[str, Any]) -> None:
        ...
        if (
            "current_project_id" in updates
            and updates["current_project_id"] is not None
        ):
    ...
    async def toggle_autopilot(self) -> bool:
        ...
        event_type = (
            ...
            if new_state
            else EventTypes.STATE_AUTOPILOT_DISABLED
        )
        await self._event_bus.publish(
            Event(
                ...
                payload={
                    "enabled": new_state,
                    "project_id": str(self._state.current_project_id)
                    if self._state.current_project_id
                    else None,
                },
            )
        )
    ...
    async def set_ai_model(self, model: str) -> None:
        ...
        await self._event_bus.publish(
            Event(
                ...
                payload={"old_model": old_model, "new_model": model},
            )
        )
    ...
    def _notify_subscribers(self) -> None:
        ...
        for callback in subscribers:
            try:
                ...
                logger.error(
                    f"Error in state change callback {callback}: {e}", exc_info=True
                )
        ...
        await self._event_bus.publish(
            Event(
                ...
                payload={
                    "changes": changes,
                    "old_state": {
                        "current_project_id": str(old_state.current_project_id)
                        if old_state.current_project_id
                        else None,
                    ...
                    "new_state": {
                        "current_project_id": str(new_state.current_project_id)
                        if new_state.current_project_id
                        else None,
    ...
    def _persist_state(self) -> None:
        ...
        try:
            ...
            # Serialize state
            state_data = {
                "current_project_id": str(self._state.current_project_id)
                if self._state.current_project_id
                else None,
                "autopilot_enabled": self._state.autopilot_enabled,
                "selected_ai_model": self._state.selected_ai_model,
