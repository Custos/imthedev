...
class EventBus:
    ...
    def __init__(self, name: str = "main"):
        ...
        """
        self.name = name
        self._handlers: dict[str, set[EventHandler] | WeakSet[EventHandler]] = {}
    ...
    def subscribe(
        self, event_type: str, handler: EventHandler, weak: bool = True
    ) -> None:
    ...
    async def _notify_handlers(self, event: Event) -> None:
        ...
        if event.type in self._async_handlers:
            tasks = []
            async_handler_set = self._async_handlers[event.type]
    ...
    async def _notify_global_handlers(self, event: Event) -> None:
        ...
        for handler in list(self._global_handlers):
            try:
                ...
                logger.error(f"Error in global handler {handler}: {e}", exc_info=True)
    ...
    async def _call_async_handler(
        ...
        try:
            ...
            logger.error(
                f"Error in async handler {handler} for {event.type}: {e}", exc_info=True
            )
    ...
    def get_handler_count(self, event_type: str | None = None) -> int:
        ...
        if event_type:
            ...
            for handlers in self._handlers.values():
                total += len(handlers)
            for async_handler_set in self._async_handlers.values():
                total += len(async_handler_set)
            return total
...
class ScopedEventBus(EventBus):
    ...
    def __init__(self, namespace: str, parent: EventBus | None = None):
