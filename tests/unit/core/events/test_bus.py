...
class TestEventBus:
    ...
    def test_bus_initialization(self, event_bus) -> None:
    ...
    @pytest.mark.asyncio
    async def test_subscribe_and_publish(self, event_bus) -> None:
        ...
        event = Event(type=EventTypes.PROJECT_CREATED, payload={"name": "Test"})
    ...
    @pytest.mark.asyncio
    async def test_async_handler(self, event_bus) -> None:
        ...
        event = Event(type=EventTypes.COMMAND_EXECUTING, payload={"command": "test"})
        await event_bus.publish(event)
    ...
    @pytest.mark.asyncio
    async def test_multiple_handlers(self, event_bus) -> None:
        ...
        event = Event(type=EventTypes.STATE_UPDATED, payload={"key": "value"})
        await event_bus.publish(event)
    ...
    @pytest.mark.asyncio
    async def test_global_handler(self, event_bus) -> None:
    ...
    @pytest.mark.asyncio
    async def test_unsubscribe(self, event_bus) -> None:
    ...
    @pytest.mark.asyncio
    async def test_error_handling(self, event_bus) -> None:
    ...
    async def test_event_history(self, event_bus) -> None:
        ...
        events = []
        for i in range(5):
            event = Event(type=EventTypes.STATE_UPDATED, payload={"index": i})
            events.append(event)
            await event_bus.publish(event)
    ...
    @pytest.mark.asyncio
    async def test_pause_and_resume(self, event_bus) -> None:
    ...
    def test_handler_count(self, event_bus) -> None:
...
class TestScopedEventBus:
    ...
    @pytest.mark.asyncio
    async def test_scoped_bus_namespace(self) -> None:
        ...
        event = Event(type=EventTypes.UI_READY, payload={})
    ...
    @pytest.mark.asyncio
    async def test_scoped_bus_local_events(self) -> None:
        ...
        local_event = Event(type="_local_worker_status", payload={"status": "busy"})
        ...
        normal_event = Event(type=EventTypes.COMMAND_COMPLETED, payload={})
