...
class TestEventTypes:
    ...
    def test_is_valid_event(self) -> None:
    ...
    def test_get_events_by_domain(self) -> None:
...
class TestEventPriority:
    ...
    def test_get_priority(self) -> None:
        ...
        # Critical events
        assert (
            EventPriority.get_priority(EventTypes.SYSTEM_ERROR)
            == EventPriority.CRITICAL
        )
        assert (
            EventPriority.get_priority(EventTypes.SYSTEM_SHUTDOWN_REQUESTED)
            == EventPriority.CRITICAL
        )
        assert (
            EventPriority.get_priority(EventTypes.COMMAND_FAILED)
            == EventPriority.CRITICAL
        )

        # High priority events
        assert (
            EventPriority.get_priority(EventTypes.UI_COMMAND_APPROVE_REQUESTED)
            == EventPriority.HIGH
        )
        assert (
            EventPriority.get_priority(EventTypes.UI_COMMAND_REJECT_REQUESTED)
            == EventPriority.HIGH
        )
        assert (
            EventPriority.get_priority(EventTypes.COMMAND_EXECUTING)
            == EventPriority.HIGH
        )

        # Low priority events
        assert (
            EventPriority.get_priority(EventTypes.STORAGE_SAVE_STARTED)
            == EventPriority.LOW
        )
        assert (
            EventPriority.get_priority(EventTypes.STATE_PERSISTED) == EventPriority.LOW
        )
        assert EventPriority.get_priority(EventTypes.CONTEXT_SAVED) == EventPriority.LOW

        # Normal priority (default)
        assert (
            EventPriority.get_priority(EventTypes.PROJECT_CREATED)
            == EventPriority.NORMAL
        )
        assert (
            EventPriority.get_priority(EventTypes.AI_GENERATION_STARTED)
            == EventPriority.NORMAL
        )
