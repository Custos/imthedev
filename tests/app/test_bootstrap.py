...
def _project_selected_init(self, project_id, project_name, project_path):
...
project_selected_mock = type(
    "ProjectSelected",
    (MockMessage,),
    {"__init__": _project_selected_init},
)
...
def _command_submitted_init(self, command, command_id):
...
command_submitted_mock = type(
    "CommandSubmitted",
    (MockMessage,),
    {"__init__": _command_submitted_init},
)
...
def _command_approved_init(self, command_id):
    self.command_id = command_id

command_approved_mock = type(
    "CommandApproved",
    (MockMessage,),
    {"__init__": _command_approved_init},
)
approval_controls_mock.CommandApproved = command_approved_mock
def _command_denied_init(self, command_id):
    self.command_id = command_id

command_denied_mock = type(
    "CommandDenied",
    (MockMessage,),
    {"__init__": _command_denied_init},
)
...
def cleanup_mocked_modules() -> None:
...
class TestImTheDevApp:
    ...
    def test_app_initialization_state(self, mock_config) -> None:
    ...
    @pytest.mark.asyncio
    async def test_successful_initialization(self, mock_config) -> None:
        """Test successful application initialization."""
        app = ImTheDevApp(mock_config)

        # Mock all the service classes to avoid actual initialization
        with patch("imthedev.app.bootstrap.EventBus") as mock_event_bus, patch(
            "imthedev.app.bootstrap.ProjectRepository"
        ) as mock_project_repo, patch(
            "imthedev.app.bootstrap.ContextRepository"
        ) as mock_context_repo, patch(
            "imthedev.app.bootstrap.AIOrchestratorImpl"
        ) as mock_ai_orchestrator, patch(
            "imthedev.app.bootstrap.CommandEngineImpl"
        ) as mock_command_engine, patch(
            "imthedev.app.bootstrap.StateManagerImpl"
        ) as mock_state_manager, patch(
            "imthedev.app.bootstrap.CoreFacade"
        ) as mock_core_facade:
            # Set up mocks
    ...
    @pytest.mark.asyncio
    async def test_initialization_failure_cleanup(self, mock_config) -> None:
        ...
        with patch("imthedev.app.bootstrap.EventBus") as mock_event_bus, patch(
            ...
            mock_event_bus.return_value = MagicMock()
            mock_project_repo.return_value.initialize.side_effect = Exception(
                "Init failed"
            )
    ...
    @pytest.mark.asyncio
    async def test_double_initialization_warning(self, mock_config) -> None:
        ...
        with patch("imthedev.app.bootstrap.EventBus") as mock_event_bus, patch(
            ...
            "imthedev.app.bootstrap.CoreFacade"
        ) as mock_core_facade, patch(
            "imthedev.app.bootstrap.logger"
        ) as mock_logger:
            # Set up mocks
            mock_event_bus.return_value = MagicMock()
            mock_project_repo.return_value = AsyncMock()
    ...
    @pytest.mark.asyncio
    async def test_run_without_initialization(self, mock_config) -> None:
        """Test that running without initialization raises error."""
        app = ImTheDevApp(mock_config)

        with pytest.raises(
            RuntimeError, match="Application must be initialized before running"
        ):
            await app.run()

    @pytest.mark.asyncio
    async def test_successful_run(self, mock_config) -> None:
    ...
    @pytest.mark.asyncio
    async def test_run_keyboard_interrupt(self, mock_config) -> None:
        """Test handling of keyboard interrupt during run."""
        app = ImTheDevApp(mock_config)
        app._initialized = True

        with patch("imthedev.ui.tui.app.ImTheDevApp") as mock_tui_app, patch.object(
            app, "cleanup", new_callable=AsyncMock
        ) as mock_cleanup:
            mock_tui_instance = AsyncMock()
    ...
    @pytest.mark.asyncio
    async def test_cleanup_partial_initialization(self, mock_config) -> None:
    ...
    @pytest.mark.asyncio
    async def test_cleanup_with_errors(self, mock_config) -> None:
...
context_dir = "/tmp/test"
    ...
    @pytest.mark.asyncio
    async def test_bootstrap_with_config_error(self) -> None:
...
default_model = "invalid-model"
    ...
    @pytest.mark.asyncio
    async def test_bootstrap_with_initialization_error(self) -> None:
...
claude_api_key = "test-key"
    ...
    @pytest.mark.asyncio
    async def test_bootstrap_default_config(self) -> None:
    ...
    @pytest.mark.asyncio
    async def test_bootstrap_creates_config_if_missing(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch("imthedev.app.bootstrap.ConfigManager") as mock_config_manager:
                ...
                with patch("sys.exit") as mock_exit:
                    ...
                    mock_manager.create_default_config_file.assert_called_once_with(
                        str(config_path)
                    )
    ...
    @pytest.mark.asyncio
    async def test_bootstrap_api_key_error_message(self) -> None:
...
class TestMain:
    ...
    @pytest.mark.asyncio
    async def test_successful_main(self) -> None:
    ...
    @pytest.mark.asyncio
    async def test_main_bootstrap_failure(self) -> None:
    ...
    @pytest.mark.asyncio
    async def test_main_run_failure(self) -> None:
        """Test main handles runtime failure."""
        with patch(
            "imthedev.app.bootstrap.bootstrap_application"
        ) as mock_bootstrap, patch("sys.exit") as mock_exit:
            mock_app = AsyncMock()
...
class TestLoggingSetup:
    ...
    def test_logging_setup_console_only(self) -> None:
    ...
    def test_logging_setup_file_and_console(self) -> None:
        ...
        with patch("logging.getLogger") as mock_get_logger, patch(
            ...
            mock_file_handler.assert_called_once_with(
                "/tmp/test.log", maxBytes=1024, backupCount=3
            )

            # Verify both handlers were added
            assert mock_logger.addHandler.call_count == 2  # Console + file

    def test_logging_setup_creates_log_directory(self) -> None:
        ...
        with patch("logging.getLogger") as mock_get_logger, patch(
            ...
            mock_log_path.parent.mkdir.assert_called_once_with(
