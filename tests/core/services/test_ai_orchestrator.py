...
from imthedev.core.interfaces import AIModel, CommandAnalysis
...
class TestMockAIAdapter:
    ...
    async def test_mock_adapter_analyze_result(self) -> None:
        ...
        cmd = Command(
            ...
            ai_reasoning="Test",
            status=CommandStatus.COMPLETED,
            result=CommandResult(exit_code=0, stdout="test", stderr="", execution_time=0.1)
        )
        ...
        # Test failed command
        cmd.result = CommandResult(exit_code=1, stdout="", stderr="error", execution_time=0.1)
...
class TestAIOrchestratorImpl:
    ...
    async def test_generate_command_events(
        ...
        # Check events
        start_events = event_collector.get_events_by_type(EventTypes.AI_GENERATION_STARTED)
        assert len(start_events) == 1
        assert start_events[0].payload["model"] == "mock-model"
        assert start_events[0].payload["objective"] == "test objective"
        
        complete_events = event_collector.get_events_by_type(EventTypes.AI_GENERATION_COMPLETED)
    ...
    async def test_generate_command_failure_event(
        ...
        # Check failure event
        fail_events = event_collector.get_events_by_type(EventTypes.AI_GENERATION_FAILED)
    ...
    async def test_invalid_model_error(
        ...
        with pytest.raises(InvalidModelError, match="Model invalid-model is not supported"):
            await orchestrator.generate_command(
                mock_context,
                "test",
                model="invalid-model"
