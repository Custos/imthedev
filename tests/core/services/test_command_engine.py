...
from imthedev.core.events import Event, EventBus, EventHandler, EventTypes
from imthedev.core.services.command_engine import CommandEngineImpl, CommandExecutionError
...
class TestCommandEngineProposal:
    ...
    async def test_propose_command_creates_pending_command(
        self,
        command_engine: CommandEngineImpl,
...
class TestCommandApproval:
    ...
    async def test_approved_command_starts_execution(
        ...
        events = event_collector.get_events_by_type(
...
class TestPendingCommands:
    ...
    async def test_get_pending_commands_returns_copy(
        ...
        command1 = await command_engine.propose_command(
            uuid4(), "echo '1'", "First"
        )
        command2 = await command_engine.propose_command(
            uuid4(), "echo '2'", "Second"
        )
