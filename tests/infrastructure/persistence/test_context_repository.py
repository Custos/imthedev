...
class TestContextRepository:
    ...
    def test_initialization(self, temp_storage_dir):
        ...
        repo = ContextRepository(temp_storage_dir)
    ...
    @pytest.mark.asyncio
    async def test_save_and_load_context(self, context_repo, sample_project_id, sample_context):
    ...
    @pytest.mark.asyncio
    async def test_load_nonexistent_context(self, context_repo):
    ...
    @pytest.mark.asyncio
    async def test_save_context_with_command_result(self, context_repo, sample_project_id):
    ...
    @pytest.mark.asyncio
    async def test_append_command_to_new_project(self, context_repo, sample_project_id, sample_command):
    ...
    @pytest.mark.asyncio
    async def test_history_size_limit(self, context_repo, sample_project_id):
        ...
        assert loaded_context.history[0].command_text == "echo 'Command 20'"  # First kept command
        assert loaded_context.history[-1].command_text == "echo 'Command 119'"  # Last command
    
    @pytest.mark.asyncio
    async def test_get_command_history(self, context_repo, sample_project_id):
        """Test retrieving command history with various filters."""
        # Create commands with different statuses
        commands = []
        statuses = [CommandStatus.PROPOSED, CommandStatus.APPROVED, CommandStatus.COMPLETED, CommandStatus.FAILED]
        ...
        # Test with limit
        limited_history = await context_repo.get_command_history(sample_project_id, limit=5)
        assert len(limited_history) == 5
        
        # Test with status filter
        completed_history = await context_repo.get_command_history(
            sample_project_id, 
            status_filter=CommandStatus.COMPLETED
        )
    ...
    @pytest.mark.asyncio
    async def test_get_command_history_ordered_by_timestamp(self, context_repo, sample_project_id):
    ...
    @pytest.mark.asyncio
    async def test_update_command_status(self, context_repo, sample_project_id, sample_command):
    ...
    @pytest.mark.asyncio
    async def test_update_command_status_not_found(self, context_repo, sample_project_id):
    ...
    @pytest.mark.asyncio
    async def test_atomic_file_operations(self, context_repo, sample_project_id, sample_context):
